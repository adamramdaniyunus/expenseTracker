...
const EditExpense = () => {
    ...
    const editExpense = async (e) => {
        e.preventDefault()
        try {
            await axiosJWT.patch(`http://localhost:5001/expense/${id}`, {
                title: title,
                amount: amount,
                type: type,
                date: date,
    ...
    const getExpenseId = async () => {
        const response = await axiosJWT.get(`http://localhost:5001/expense/${id}`, {
            headers: {
                Authorization: `Bearer ${token}`
            }
        })
    ...
    return (
        <div className='mt-14 flex justify-center'>
            <form className='w-full mx-4' onSubmit={editExpense}>
